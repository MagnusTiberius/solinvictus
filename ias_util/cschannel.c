    ...
    PSecBufferDesc put, PSecBufferDesc get )
{
    ...
    if ( FAILED(result) )
    {
		printf("Error in AcceptSecurityContext()\n");
		if (result == SEC_E_NO_AUTHENTICATING_AUTHORITY) {
			printf("Error in AcceptSecurityContext() SEC_E_NO_AUTHENTICATING_AUTHORITY \n");
		}
		if (result == SEC_E_NO_CREDENTIALS) {
			printf("Error in AcceptSecurityContext() SEC_E_NO_CREDENTIALS \n");
		}
		if (result == SEC_E_UNSUPPORTED_FUNCTION) {
			printf("Error in AcceptSecurityContext() SEC_E_UNSUPPORTED_FUNCTION \n");
		}
		if (result == SEC_E_INSUFFICIENT_MEMORY) {
			printf("Error in AcceptSecurityContext() SEC_E_INSUFFICIENT_MEMORY \n");
		}
		if (result == SEC_E_INTERNAL_ERROR) {
			printf("Error in AcceptSecurityContext() SEC_E_INTERNAL_ERROR \n");
		}
		if (result == SEC_E_INVALID_HANDLE) {
			printf("Error in AcceptSecurityContext() SEC_E_INVALID_HANDLE \n");
		}
		if (result == SEC_E_LOGON_DENIED) {
			printf("Error in AcceptSecurityContext() SEC_E_LOGON_DENIED \n");
		}
		if (result == SEC_E_NO_AUTHENTICATING_AUTHORITY) {
			printf("Error in AcceptSecurityContext() SEC_E_NO_AUTHENTICATING_AUTHORITY \n");
		}
		if (result == SEC_E_APPLICATION_PROTOCOL_MISMATCH) {
			printf("Error in AcceptSecurityContext() SEC_E_APPLICATION_PROTOCOL_MISMATCH \n");
		}
		if (result == SEC_I_COMPLETE_AND_CONTINUE) {
			printf("Error in AcceptSecurityContext() SEC_I_COMPLETE_AND_CONTINUE \n");
		}
		if (result == SEC_I_CONTINUE_NEEDED) {
			printf("Error in AcceptSecurityContext() SEC_I_CONTINUE_NEEDED \n");
		}
		if (result == SEC_E_INCOMPLETE_MESSAGE)
        {
            channel->need_push = 1;
