...
void DirChange::AddDirectory(std::wstring szDirectory, BOOL bWatchSubtree, DWORD dwNotifyFilter)
{
	...
	while (!bTerminate)
	{
		...
		case WAIT_OBJECT_0 + 0:
			...
			if (TryGetKeyboardInput(hStdIn, bTerminate, buf, MAX_PATH)){
				...
				changes.AddDirectory(std::wstring(str.begin(), str.end()), false, dwNotificationFlags);
			}
			break;
...
bool DirChange::TryGetKeyboardInput(HANDLE hStdIn, bool &bTerminate, char* buf, size_t siz)
{
	...
	if (rec.EventType == KEY_EVENT)
	{
		...
		{
			if (!gets_s(buf, siz))	// End of file, usually Ctrl-Z
				bTerminate = true;
			else
				return true;
		}
